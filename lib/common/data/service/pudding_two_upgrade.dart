...
import 'package:pudding/common/data/models/pudding_two_upgrade.dart';
import 'package:pudding/common/data/service/dio.dart';
import 'package:pudding/common/data/service/secure_storage.dart';

class PuddingUpgradeTwo {
  Future<PuddingTwoUpgrade> puddingUpgradeTwo() async {
    try {
      ...
      final response = await dio.post(
        ApiEndpoints.upgradeTwo,
        options: Options(headers: {'Authorization': 'Bearer $accessToken'})
      );

      if (response.statusCode == 201) {
        return PuddingTwoUpgrade.fromJson(response.data);
      }
      else {
